fn main() {
    // exp変数をRPN形式(Reverse Polish Notation: 逆ポーランド記法)の文字列に束縛する
    ...
    // デバッグビルド時のみ答えが正しいかチェックする．浮動小数点の計算誤差を考慮し，ここでは小数点以下4桁までの値を文字列に変換している
...
fn rpn(exp: &str) -> f64 {
    // 変数stackを空のスタックに束縛する．stackはミュータブル(mutable: 可変)な変数で，値の変更を許す
    let mut stack = Vec::new();

    // expの要素をスペースで分割し，tokenをそれらに順に束縛する．要素がなくなるまで繰り返す
    for token in exp.split_whitespace() {
        ...
        if let Ok(num) = token.parse::<f64>() {
            ...
            match token {
                ...
                // |x, y| x + y はクロージャ．引数 x, y を取り，x + y を計算して答えを返す
